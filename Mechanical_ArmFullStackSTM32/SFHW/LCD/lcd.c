...
u16 LCD_ReadPoint(u16 x,u16 y)
{
	...
	if(lcddev.id==0X9341||lcddev.id==0X6804
		||lcddev.id==0X5310||lcddev.id==0X1963)LCD_WR_REG(0X2E);//9341/6804/3510/1963 发送读GRAM指令
	...
	if(lcddev.id==0X9325||lcddev.id==0X4535
		||lcddev.id==0X4531||lcddev.id==0XB505
	||lcddev.id==0XC505){
...
{  							  
	...
	for(t=0;t<csize;t++)
	{   
		if(size==12)temp=asc2_1206_[num][t]; 	 	//调用1206字体
		else if(size==16)temp=asc2_1608_[num][t];	//调用1608字体
		else if(size==24)temp=asc2_2412_[num][t];	//调用2412字体
		else return;								//没有的字库
		for(t1=0;t1<8;t1++)
		{			    
...
void OLED_Show_Font(u16 x,u16 y,u8 fnum)
{
	...
	u16 y0=y;
	u8 *dzk;   
	int csize=350;					//一个24*24的汉字72字节
	dzk=(u8*)OLED_HZK_50_[fnum];	//得到汉字编号对应的点阵库 
	for(t=0;t<csize;t++)
	{   												   
		temp=dzk[t];				//得到点阵数据                          
		for(t1=0;t1<8;t1++)
